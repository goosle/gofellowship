...
if ($islogin&& isset($_GET['me']))
{
	$sql ='select t.* ,"'.$userdata['username'].'" as username  from  '.TOPICS_TABLE .' t  where t.topic_poster = '
	.$userdata['user_id']." AND t.forum_id = $forum_id"." LIMIT $start, ".$board_config['topics_per_page'];; 
}
else
if($islogin && isset($_GET['join']))
{
	$sql = ' select t.* , u.username from  '.TOPICS_TABLE.' t ,'.TOPICS_JOIN_TABLE.' tj ,' .USERS_TABLE
	." u  where tj.user_id=".$userdata['user_id']." AND t.forum_id=".$forum_id.
	" and  tj.topic_id = t.topic_id and t.topic_poster = u.user_id ".
	" LIMIT $start, ".$board_config['topics_per_page'];; 
}
else
{
	$sql ='select t.* ,u.username  from  '.TOPICS_TABLE ." t ,".USERS_TABLE." u  where  t.forum_id =".$forum_id.
	" AND u.user_id = t.topic_poster  ORDER BY t.topic_type DESC, t.topic_last_post_id DESC "
	."	LIMIT $start, ".$board_config['topics_per_page'];;	
}
if ( !($result = $db->sql_query($sql)) )
{
