...
//edit ,new ,group_update ,other 响应
if ( isset($_GET['edit']) || isset($_POST['edit']) || isset($_POST['new']) )
{ 
	$template->set_filenames(array(
		'body' => 'user_group_edit_body.tpl')
	);
	...
	if (isset($_GET['edit']) || isset($_POST['edit']) )
	{
		...
		{//dejiang check if is group moderator
			trigger_error('不是的小组管理员', E_USER_ERROR); 
		}
		$mode = 'editgroup';
	...
	$template->assign_vars(array(
		...
		'USER_GROUPS'				=> append_sid('groupcp.php'),
		...
		'S_GROUP_ACTION' 				=> append_sid('user_groups.php'),
...
else if ( isset($_POST['group_update']) )
{
	if ( isset($_POST['group_delete']) )
	{
		...
		$message = '删除成功<br />点击 <a href="' . append_sid('groupcp.php') . '">这里</a> 返回小组列表<br />';
	...
	else
	{
		...
		$group_moderator 		= $userdata["username"];
		$delete_old_moderator 	=  false;
		$group_logo 			=  '';
		...
		$group_moderator = $userdata['user_id'];
		...
		if( $mode == 'editgroup' )
		{
			...
			$message = '小组信息成功更新<br />点击 <a href="' . append_sid('groupcp.php') . '">这里</a> 返回小组列表<br />';
		...
		else if( $mode == 'newgroup' )
		{
			...
			$message = '创建成功<br />点击 <a href="' . append_sid('groupcp.php') . '">这里</a> 返回小组列表<br />';
