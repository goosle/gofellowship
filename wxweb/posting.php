...
//dejiang apped group_id
$params = array('forum_id' => POST_FORUM_URL,"group_id"=>POST_GROUPS_URL, 'topic_id' => POST_TOPIC_URL, 'post_id' => POST_POST_URL, 'user_otv_id' => POST_USERS_URL);
...
if ( isset($_POST['cancel']) )
{
	...
	//dejiang append group_redirect
	else if( $group_id )
	{
		$redirect = 'viewforum_group.php?' . POST_FORUM_URL . '= 0 &'.POST_GROUPS_URL."=$group_id";
		$post_append = '';
	}
	else if ( $forum_id )
	{
...
//fetch post ,topic edit information
$error_msg = '';
$post_data = array();
switch ( $mode )
{
	...
	case 'poll_delete':
		...
		$select_sql = (!$submit) ? ', t.topic_title, t.topic_hide_author,p.enable_bbcode, p.enable_html, p.enable_smilies, p.enable_sig, p.post_username, pt.post_subject, pt.post_text, pt.bbcode_uid, u.username, u.user_id, u.user_sig, u.user_sig_bbcode_uid' : '';
...
if ( ($result = $db->sql_query($sql)) && ($post_info = $db->sql_fetchrow($result)) )
{
    ...
    if( $mode == 'newtopic')
	{//dejiang add newtopic set group_id
        
        $post_data["topic_group_id"]= $group_id;
	}
...
if ( !$is_auth_type )
{
	...
	{
		case 'newtopic':
		    if($group_id)//dejiang append to group new topic
			{
				$redirect = 'mode=newtopic&' . POST_FORUM_URL . '=' . $forum_id."&".POST_GROUPS_URL."=$group_id";
			}
			else
			{
	...
	switch ( $mode )
	{
		...
		case 'reply':
			...
			if ( $error_msg == '' )
			{
				...
				submit_post($mode, $post_data, $return_message, $return_meta, $forum_id, $topic_id, $post_id, $poll_id, $topic_type, $topic_marrow, $bbcode_on, $html_on, $smilies_on, $attach_sig, $bbcode_uid, str_replace("\'", "''", $username), str_replace("\'", "''", $subject), $hide_author,str_replace("\'", "''", $message), str_replace("\'", "''", $poll_title), $poll_options, $poll_length);
			}
			break;
...
switch( $mode )
{
	case 'newtopic':
		...
		if(!empty($group_id))
			$hidden_form_fields .= '<input type="hidden" name="' . POST_GROUPS_URL . '" value="' . $group_id . '" />';
	break;
...
else
{
	if($group_id)
	{
		$back_url = append_sid('viewforum_group.php?' . POST_FORUM_URL . '=' . $forum_id."&".POST_GROUPS_URL."=$group_id");
	}
	else 
	{
...
if ( $userdata['session_logged_in'] && $userdata['user_bb_panel'] )
{
 ...
 $hide_author = $post_info['topic_hide_author'];
$template->assign_vars(array(
	'USERNAME' 			=> $username,
	'SUBJECT' 			=> $subject,
	'MESSAGE' 			=> $message,
	...
	'S_HIDE_AUTHOR'		=> ( $hide_author==1 ) ? 'checked="checked"' : '',
