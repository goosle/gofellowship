...
$is_moderator = FALSE;
//group stattus post, update group type
...
{//添加成员，
	...
	$template->assign_vars(array(
		...
		'U_GROUP_CP'                 =>"groupcp.php",//dejiang apppend to head title url
	...
	if ( $userdata['session_logged_in'] ) 
	{
		...
		if ( $row = $db->sql_fetchrow($result) )
		{
			...
			do
			{
				...
				{
					$s_member_groups_opt 	.= '- <a href="' . append_sid('groupcp.php?' . POST_GROUPS_URL . '=' . $row['group_id'] . '&amp;sid=' . $userdata['session_id']) . '">' . $row['group_name'] . '</a>';
				    //dejiang 添加查看贴贴和修改组
					$s_member_groups_opt 	.= '&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp <a href="' . append_sid('viewforum_group.php?f='.FORUM_GROUP_ID.'&'. POST_GROUPS_URL . '=' . $row['group_id'] . '&amp;sid=' . $userdata['session_id']) . '">' . '帖子' . '</a>';
				    if($row['group_moderator'] == $userdata['user_id'])
					{
						$s_member_groups_opt 	.= '&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp <a href="' . append_sid('user_groups.php?f='.FORUM_GROUP_ID.'&'. POST_GROUPS_URL . '=' . $row['group_id'] . '&amp;sid=' . $userdata['session_id']) . '&edit">' . '修改' . '</a><br/>';
				    }
					else
					{
						$s_member_groups_opt 	.= '<br/>';
					}
...
define('NEW_GROUP_MIN_POST', 2);
	if ( $s_group_list_opt != '' || $s_pending_groups_opt != '' || $s_member_groups_opt != '' )
	{
		...
		//dejiang append ,new group 
        if( $userdata['user_level'] == ADMIN || $userdata['user_level'] == MOD 
		           || $userdata["user_posts"]>NEW_GROUP_MIN_POST)
		{
			$template->assign_block_vars('new_groups',array());
		}
		$s_hidden_fields = '<input type="hidden" name="sid" value="' . $userdata['session_id'] . '" />';

		$template->assign_vars(array(
		    'S_NEW_GROUP_ACTION'            => append_sid("user_groups.php"),
