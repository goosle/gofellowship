...
if($group_id)
{
	$group_where = " and topic_group_id=$group_id ";
}
else
	$group_where = '';
$sql = "SELECT t.topic_id, t.topic_title ,u.username  FROM "
  
  .TOPICS_TABLE." t  left join ".USERS_TABLE." u on t.topic_poster=u.user_id  WHERE forum_id = $forum_id ".$group_where.
	" AND ( t.topic_title LIKE '%$keyword%' or u.username like '%$keyword%' ) " 
    ." ORDER BY topic_time DESC LIMIT $start, $per";
...
$sql = "SELECT COUNT(topic_id) AS total
	FROM " . TOPICS_TABLE . "
	WHERE forum_id = $forum_id".$group_where.
...
if($group_id)
{
	$u_back = append_sid(ROOT_PATH . 'viewforum_group.php?' . POST_FORUM_URL . '=' . $forum_id."&".POST_GROUPS_URL.'='.$group_id);
}
else
{
