...
class Session
{
	...
	//if user login from wx ,will set the value
	var $openID = '';
	...
	*/
	function init()
	{
		...
		// 如果Cookie存在则优先使用get,not Cookie，否则使用GET
		if(isset($_GET['sid']) )
		{
	...
	function update($user_id, $user_ip, $page_id, $auto_create = 0, $enable_autologin = 0, $admin = 0)
	{
		...
		if ( !$this->_db->sql_query($sql) || !$this->_db->sql_affectedrows() )
		{
			...
			$sql = "INSERT INTO " . SESSIONS_TABLE . "
				(session_id, session_user_id, session_start, session_time, session_ip, session_page, session_logged_in, session_admin,session_open_id)
				VALUES ('$this->session_id', $user_id, $this->_time, $this->_time, '$user_ip', $page_id, $login, $admin,'$this->openID')";
		...
		$this->userdata['session_open_id']=$this->openID;
	...
	*/
	...
	//*********************************************************
	//set sesssion by open id
	
	/**
	*	Session start
	*/
	function startByOid($user_ip,$page_id)
	{
		...
		if ( !($this->userdata = $this->updateByOid($user_id, $user_ip, $this->page_id, TRUE)) )
		{
			trigger_error('无法创建用户Session信息', E_USER_WARNING);
		}
	...
	*	用于用户登录成功后的Session更新
	*/
	function updateByOid($user_id, $user_ip, $page_id, $auto_create = 0, $enable_autologin = 0, $admin = 0)
	{
		if (isset($this->config['allow_autologin']) && !$this->config['allow_autologin'])
		{
			$enable_autologin = $this->session_data['autologinid'] = false;
		...
		$sql = "UPDATE " . SESSIONS_TABLE . "
			SET session_user_id = $user_id, session_start = $this->_time, session_time = $this->_time, session_page = $page_id, session_logged_in = $login, session_admin = $admin
			WHERE session_id = '" . $this->session_id . "' 
				AND session_open_id = '$this->openID'";
