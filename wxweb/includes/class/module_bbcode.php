...
class Module_bbcode 
{
	...
	{
		global $db;
		//gsl_log_info("1=".$topic[1].',2='.$topic[2].',3='.$topic[3].',4='.$topic[4].',5='.$topic[5].',6='.$topic[6]);
		$module_text = '';
		$while_br = ($topic[3]) ? '<br />' : '';
		...
		switch ($topic[4])
		{
			...
			case 2:
				while( $row = $db->sql_fetchrow($result) )
				{
					$module_text .= '<a href="' . append_sid(ROOT_PATH . 'viewtopic.php?t=' . $row['topic_id']) . '">'.$row['forum_name'].'---' . $row['topic_title'] . '</a>' . $while_br;
				}
				break;
