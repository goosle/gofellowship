...
function submit_post($mode, &$post_data, &$message, &$meta, &$forum_id, &$topic_id, &$post_id, &$poll_id, &$topic_type, &$topic_marrow, &$bbcode_on, &$html_on, &$smilies_on, &$attach_sig, &$bbcode_uid, $post_username, $post_subject, $hide_author,$post_message, $poll_title, &$poll_options, &$poll_length)
{
	...
	{//topic table update
        ...
        //dejiang 添加 topic_group_id
		$group_id =empty($post_data["topic_group_id"])?0:$post_data["topic_group_id"];
        if($mode != "editpost")     		
		{
			$sql  =  "INSERT INTO " . TOPICS_TABLE . " (topic_group_id,topic_title,topic_hide_author, topic_poster, topic_time, forum_id, topic_status, topic_type, topic_marrow, topic_vote) "
			        ." VALUES ($group_id,'$post_subject',$hide_author, " . $userdata['user_id'] . ", $current_time, $forum_id, " . TOPIC_UNLOCKED . ", $topic_type, $topic_marrow, $topic_vote)" ;
		}
        else
		{
			$sql =  "UPDATE " . TOPICS_TABLE . " SET topic_title = '$post_subject', topic_hide_author=$hide_author,topic_type = $topic_type, topic_marrow = $topic_marrow " . (($post_data['edit_vote'] || !empty($poll_title)) ? ", topic_vote = " . $topic_vote : "") . " WHERE topic_id = $topic_id";
		}
		if (!$db->sql_query($sql))
		{
