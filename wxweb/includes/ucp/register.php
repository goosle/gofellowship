...
if ( isset($_POST['submit']) )
{
	...
	{
		if ($username == '' || $new_password == '' || $password_confirm == '' || $email == '' )
		{
			$error = TRUE;
			$error_msg .= "<p>标 * 选项不能留空$username,$new_password,$password_confirm,$email,$gender </p>";
		}
	}
	...
	if ( !$error )
	{
		...
		else
		{
			...
			// 注册时执行的数据
			$sql = "INSERT INTO " . USERS_TABLE . "	(user_id, username,user_openID, user_regdate, user_password, user_email, user_viewemail, user_attachsig, user_allowsmile, user_allowhtml, user_allowbbcode, user_allow_viewonline, user_notify_to_email, user_notify_to_pm, user_notify_pm, user_popup_pm, user_timezone, user_dateformat, user_level, user_allow_pm, user_message_quote, user_topics_per_page, user_posts_per_page, user_posl_red, user_index_spisok, user_style, user_gender, user_active, user_actkey) 
				VALUES ($user_id, '" . $db->sql_escape($username) . "','".$userdata['session_open_id']."',". time() . ", '" . $db->sql_escape($new_password) . "', '" . $db->sql_escape($email) . "', 1, 0, 1, 0, 1, 1, 0, 0, 0, 1, $board_timezone, '" . $db->sql_escape($default_dateformat) . "', 0, 1, '" . $board_config['message_quote'] . "', '$topics_per_page', '$posts_per_page', '" . $board_config['posl_red'] . "', '" . $board_config['index_spisok'] . "', " . $board_config['default_style'] . ",".$gender.",";
