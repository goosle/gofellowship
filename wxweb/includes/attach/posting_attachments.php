...
class attach_posting extents attach_parent
{
...
function execute_posting_attachment_handling()
该函数在posting.php :400 call execute_posting_attachment_handling();
***************/
...
//1 从附件表获取已上传附件信息
	...
	function display_attachment_bodies()
	{
		...
		if (count($this->attachment_list) > 0)
		{
			...
			for ($i = 0; $i < count($this->attachment_list); $i++)
			{
				...
				//添加了attachment_bbcode_type 变量
				$extName = $this->attachment_extension_list[$i];
				$types = array ("jpg","jpeg","gif","png","bmp");
				$hasBBType = false;
				for($j=0;$j<count($types);++$j)
				{
				   if(strcasecmp($extName,$types[$j])==0)
				   {
					  $this->attachment_bbcode_type[$i]='img';
					  $hasBBType = true;
                      break;
				   }
				}
				if(!$hasBBType )
				{
					$types = array ("mp3","amr","m4a");
					for($j=0;$j<count($types);++$j)
					{
						if(strcasecmp($extName,$types[$j])==0)
						{
							$this->attachment_bbcode_type[$i]='audio';
							$hasBBType = true;
							break;
						}
					}
				...
				if(!$hasBBType )
				{
					$types = array ("mp4","3gp","m4v");
					for($j=0;$j<count($types);++$j)
					{
						if(strcasecmp($extName,$types[$j])==0)
						{
							$this->attachment_bbcode_type[$i]='video';
							$hasBBType = true;
							break;
						}
					}
				...
				if(!$hasBBType )
				{
					...
					for($j=0;$j<count($types);++$j)
					{
						if(strcasecmp($extName,$types[$j])==0)
						{
							$this->attachment_bbcode_type[$i]='flash';
							$hasBBType = true;
							break;
						}
					}
				...
				if(substr($this->attachment_mimetype_list[$i],0,5)=='audio')
					$this->attachment_bbcode_type[$i]='audio';
				else
				if(substr($this->attachment_mimetype_list[$i],0,5)=='video')
					$this->attachment_bbcode_type[$i]='video';
				else
				if(substr($this->attachment_mimetype_list[$i],0,5)=='appli')
					$this->attachment_bbcode_type[$i]='flash';
				else
					$this->attachment_bbcode_type[$i]='';
				*/
			}
		}
		...
		if ($this->posted_attachments_body && count($this->attachment_list) > 0)
		{
			...
			for ($i = 0; $i < count($this->attachment_list); $i++)
			{
				...
				$template->assign_block_vars('attach_row', array(
                    ...
                    'ATTACH_BBCODE_TYPE' => $this->attachment_bbcode_type[$i],  
	...
	function upload_attachment()
	{
		...
		if ($this->post_attach) 
		{
			...
			$sql = 'SELECT g.group_id,g.allow_group, g.max_filesize, g.cat_id, g.forum_permissions
			...
			//gsl_log_info("allow file size =$allowed_filesize:".$row['max_filesize'].":".$board_config['max_filesize']);
			...
			if ($cat_id == IMAGE_CAT || strpos($this->type, 'image/') === 0)
			{
				...
				{
					$error = TRUE;
					$error_msg .= '<p>无法上传文件到 ./' . $upload_dir . '/' . create_date('Ym', $this->filetime, 0) . '/' . $this->attach_filename . " 目录</p>,cat_id=$cat_id,imginfo = false";
				}
				else
				{
					$types = array(
					...
					{
						$error = TRUE;
						$error_msg .= '<p>无法上传文件到 ./' . $upload_dir . '/' . create_date('Ym', $this->filetime, 0) . '/' . $this->attach_filename . " 目录</p>,cat_id=$cat_id,image_info[2]";
					}
		...
		switch ($upload_mode)
		{
			case 'copy':
				...
				if (!@copy($file, $upload_dir . '/' . create_date('Ym', $this->filetime, 0) . '/' . basename($this->attach_filename)))
				{
					...
					{
						$error = TRUE;
						$error_msg .= '<p>无法上传文件到 ./' . $upload_dir . '/' . create_date('Ym', $this->filetime, 0) . '/' . $this->attach_filename . ' 目录</p>,copy_upload fail';
						return;
					}
				} 
			...
			case 'move':
				...
				{ 
					...
					{
						$error = TRUE;
						$error_msg .= '<p>无法上传文件到 ./' . $upload_dir . '/' . create_date('Ym', $this->filetime, 0) . '/' . $this->attach_filename . ' 目录</p>,move upload fail';
						return;
					}
				} 
