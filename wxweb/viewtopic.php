...
$sql = 'SELECT t.topic_id, t.topic_group_id,t.topic_title,t.topic_hide_author, t.topic_poster, t.topic_status, t.topic_replies, t.topic_views, t.topic_time, t.topic_type, t.topic_marrow, t.topic_vote, t.topic_last_post_id, t.topic_closed, f.forum_name, f.forum_status, f.forum_id, f.auth_view, f.auth_read, f.auth_post, f.auth_reply, f.auth_edit, f.auth_delete, f.auth_sticky, f.auth_announce, f.auth_marrow, f.auth_pollcreate, f.auth_vote, f.auth_attachments, f.auth_download, t.topic_attachment' . $count_sql . '
...
if ( $download )
{
	...
	while ( $row = $db->sql_fetchrow($result) )
	{
		...
		$poster = ( $poster_id == ANONYMOUS )||($row['username']) ? '匿名用户' : $row['username'];
...
//dejiang check group topic
//获取帖子group
$topic_group_id = $forum_topic_data['topic_group_id'];//所属小组，没有小组=0
if($topic_group_id)
{
	$user_id = $userdata["user_id"];
	$sql = " select g.group_id,g.group_name FROM ".USER_GROUP_TABLE. " ug ,".GROUPS_TABLE." g where ug.group_id=g.group_id and ug.group_id = $topic_group_id and ug.user_id= $user_id  ";
	if ( !($result = $db->sql_query($sql)) )
	{
		trigger_error('Could not obtain topic information', E_USER_WARNING);
	}
	...
	$topic_group_name = $row["group_name"];
}
...
if($topic_group_id)
{//小组帖
	$u_forum = append_sid('groupcp.php');
	$forum_center_name= "小组";
	$view_forum_url = append_sid("viewforum_group.php?f=1000&g=$topic_group_id");
	$forum_name = $topic_group_name;	
}
else
{//论坛帖
	$u_forum = append_sid('forum.php');
	$forum_center_name= "论坛中心";
}

$template->assign_vars(array(
	'FORUM_ID' 			=> $forum_id,
	'FORUM_NAME' 		=> $forum_name,
	'FURUM_CENTER_NAME' => $forum_center_name,
	'TOPIC_ID' 			=> $topic_id,
...
for($i = 0; $i < $total_posts; $i++)
{
	...
	// 用户名颜色
	$hide_author = ($i==0)&& isset($forum_topic_data['topic_hide_author'])&& ($forum_topic_data['topic_hide_author']==1);
	$post_user_name =$hide_author ?"匿名": $postrow[$i]['username'];
  
	if ( $postrow[$i]['user_warnings'] == 0 )
	{
		if ( !empty($postrow[$i]['user_nic_color']) )
		{
	$poster = ( $poster_id == ANONYMOUS || $hide_author )
			   ?( ($postrow[$i]['post_username'] != '' )    ? $postrow[$i]['post_username'] : '匿名' )
			   : '<a href="' . append_sid("ucp.php?mode=viewprofile&amp;" . POST_USERS_URL . '='  . $postrow[$i]['user_id']) . '" style="color: '.$postrow[$i]['user_nic_color'].'">' .$post_user_name  . '</a>';
		}
		else
		{
			$poster = ( $poster_id == ANONYMOUS || $hide_author  ) 
			   ? ( ($postrow[$i]['post_username'] != '' ) ? $postrow[$i]['post_username'] : '匿名' )
			   : '<a href="' . append_sid("ucp.php?mode=viewprofile&amp;" . POST_USERS_URL . '='  . $postrow[$i]['user_id']) . '">' . $post_user_name  . '</a>';
		}
	...
	else 
	{
		$poster = ( $poster_id == ANONYMOUS || $hide_author ) 
		      ? ( ($postrow[$i]['post_username'] != '' ) ? $postrow[$i]['post_username'] : '匿名' ) 
		      : '<a href="' . append_sid("ucp.php?mode=viewprofile&amp;" . POST_USERS_URL . '='  . $postrow[$i]['user_id']) . '" style="color:#000000">' .$post_user_name  . '</a>';
	}
	...
	switch ($nomer_posta)
	{
		case 1:
			...
			$reply = '';
			$quote = '';
			if ($poster_id == ANONYMOUS || $hide_author)
			{
				$poster_info = '';
			}
			else
			...
			//$download_topic = ($userdata['session_logged_in']) ? '<br />您可以 <a href="' . append_sid("viewtopic.php?download=-1&amp;".POST_TOPIC_URL."=".$topic_id) . '">下载</a>  . <a href="' . append_sid('ucp.php?mode=topic_collect&action=add&' . POST_USERS_URL . '=' . $userdata['user_id'] . '&tc=' . $topic_id) . '">收藏</a> 这个主题哦' : '';
			 ...
			 if( isset($_GET['join']) )
   			{
					$set_join_sql='insert into '.TOPICS_JOIN_TABLE .' (topic_id,user_id) value('.$topic_id.','.$userdata['user_id'].')';
					$db->sql_query($set_join_sql);
			}	
				
			$join_sql = 'SELECT u.user_id,u.username from '.TOPICS_JOIN_TABLE . ' t left join '.USERS_TABLE .' u on t.user_id = u.user_id  WHERE t.topic_id = ' . $topic_id ;
			if ( !($result = $db->sql_query($join_sql)) )
			{
				trigger_error('Could not obtain topic watch information', E_USER_WARNING);
			}
			$join_users;
			$is_joined = 0;
        	 ...
        	 while ( $row = $db->sql_fetchrow($result) )
			{
				 //info_log($row['user_id'].' '.$userdata['user_id']);
				    if($is_joined==0 &&($row['user_id'] == $userdata['user_id']))
					{
						$is_joined = 1;
					}
		            $join_users.='  '.$row['username'];
			}
            if(!empty($join_users))
                     $join_users ='<br />'.$join_users;
            if($is_joined == 0)
			{
               				
				$join_member = ($userdata['session_logged_in']) ? '您可以 <a href="'  . append_sid("viewtopic.php?join=1&amp;".POST_TOPIC_URL."=".$topic_id) 
				   . '">加入'.$g_forum_module_names[$forum_id].'</a><div>'.$join_users.'</div>':'';
					//. <a href="' . append_sid('ucp.php?mode=topic_collect&action=add&' . POST_USERS_URL . '=' . $userdata['user_id'] . '&tc=' . $topic_id) . '">收藏</a> 这个主题哦' : '';
			}
			else
			{
				$join_member = '<div>已加入'.$g_forum_module_names[$forum_id].'成员:'.$join_users.'</div>';
			}
			$download_topic ='';				 
	        //***************************************************************************
			$topic_mod		= $topic_mod;
